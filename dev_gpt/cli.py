...
def path_param(func):
    @click.option(
        "--path", default="microservice", help="Path to the generated microservice."
    )
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
...
def generate(
    ...
    if os.path.exists(path):
        if os.listdir(path):
            click.echo(
                f"Error: The path {path} you provided via --path is not empty. Please choose a directory that does not exist or is empty."  # noqa: E501
            )
            return
